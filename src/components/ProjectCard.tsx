      ...
      >
        ...
        <div className={`md:col-span-7 ${index % 2 === 0 ? '' : 'md:order-2'}`}>
          <div className="relative group rounded-2xl overflow-hidden shadow-xl">
            <div className="absolute inset-0 bg-gradient-to-br from-orange-500/80 via-amber-500/80 to-yellow-500/80 mix-blend-multiply group-hover:opacity-0 transition-opacity duration-300" />
        ...
        >
          <p className="text-orange-500 dark:text-orange-400 font-mono text-sm mb-2">Featured Project</p>
          <h3 className="text-xl font-bold text-dark-800 dark:text-dark-100 mb-1">{project.title}</h3>
          <p className="text-amber-500 dark:text-amber-400 text-sm mb-3">{project.subtitle}</p>
          ...
          <ul className={`flex flex-wrap gap-2 mb-4 text-xs font-mono text-dark-500 dark:text-dark-400 ${
            ...
            {project.technologies.slice(0, 5).map((tech) => (
              <li key={tech} className="px-2 py-1 bg-orange-50 dark:bg-orange-950/30 rounded text-orange-600 dark:text-orange-400">{tech}</li>
            ))}
          </ul>
